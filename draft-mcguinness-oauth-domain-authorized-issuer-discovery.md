...
already deploy for CAA, MTA-STS, SPF, and DKIM. The same records
that let a Resource Authorization Server verify an assertion's
issuer also let a client discover which Assertion Issuer is
...
OAuth deployments using identity-assertion grants ({{TRUST-POLICY}})
...
The same policy answers two related questions about a subject
namespace:

1. **Verification.** Given an identity assertion, is the issuer
   authorized to assert about this subject?

2. **Assertion Issuer discovery.** Given a subject identifier,
   which authorization server is authoritative for assertions about
   this subject?

Verification is defined in {{dii-verification}}; Assertion Issuer
discovery in {{dii-hrd}}. Both consumers use the same records and
the same document.
...
covered in {{TRUST-POLICY}} (§Transitive Authorization is Bounded)
and {{AUTHORITY-DELEGATION}} (§Bounded vs Chained Transitivity).
...
DAI is a concrete profile of {{AUTHORITY-DELEGATION}} for OAuth
...
DAI is consumed by {{TRUST-POLICY}}, which owns the Trust Policy
...
This document uses terminology from {{TRUST-POLICY}} (Resource
Authorization Server, Assertion Issuer, Subject Authority, Trust
Policy, Issuer Authorization Policy) and from {{AUTHORITY-DELEGATION}}
...
: The process by which a client uses DAI records to determine
which Assertion Issuer is authoritative for a subject
identifier's namespace ({{dii-hrd}}).
...
: REQUIRED. String. The Subject Authority identifier this policy
  ...
  Authority computed in {{TRUST-POLICY}} §Subject Authority Determination after applying the
...
the syntax and recognition rules in {{TRUST-POLICY}} §Critical Extension Identifiers.

`signed_policy`
: OPTIONAL. Signed JWT containing policy members as claims, using the
  representation defined in {{TRUST-POLICY}} §Signed Policy Metadata. This member
...
| `signed_policy` | string, optional | signed JWT as defined in {{TRUST-POLICY}} §Signed Policy Metadata |
...
| 3: Signed Policy at Arbitrary Origin | TXT with `uri=` | HTTPS-hosted JSON with `signed_policy` ({{TRUST-POLICY}} §Signed Policy Metadata); MAY be at any HTTPS origin | DNS control of `{A}` AND JWS signature binding the signer to the Subject Authority | Third-party hosting, CDN mirroring, disaster recovery, transparency-log integration |

A Subject Authority using Profile 3 (Signed Policy at Arbitrary
Origin) MUST publish `signed_policy` in the served document satisfying
the binding rules of
{{TRUST-POLICY}} §Signed Policy Metadata. Consumers retrieving a document from a host
NOT under the Subject Authority's DNS-namespace control MUST verify
`signed_policy` before relying on the document's contents. Profiles 1
and 2 do not require `signed_policy` because the authority binding is
established by channel control (DNS
...
: REQUIRED. The Subject Authority identifier this record claims to
  ...
  case-insensitive ASCII comparison rules in {{TRUST-POLICY}} §Subject Authority Determination.
...
2. If the DNS response is `affirmative`:
   ...
   a. Discard recognized records whose `authority=` directive does
      ...
      {{TRUST-POLICY}} §Subject Authority Determination. If any recognized record is missing an
...
({{AUTHORITY-DELEGATION}} §Exception-Handling and Fallback Model). The normative
...
1. Determine the Subject Authority from the assertion's Subject
   Identifier per {{TRUST-POLICY}} §Subject Authority Determination. If the format is not registered
   in {{TRUST-POLICY}} §Subject Authority Extraction Procedures Registry, reject the assertion.
...
{{TRUST-POLICY}} §Resource Authorization Server Processing is not met.
...
1. Determine the Subject Authority `A` from the input subject
   identifier per {{TRUST-POLICY}} §Subject Authority Determination.
...
Authority determination rules ({{TRUST-POLICY}} §Subject Authority Determination), HTTPS document URL
...
1. Determine the Subject Authority `A` from the assertion's Subject
   Identifier per {{TRUST-POLICY}} §Subject Authority Determination. If the format is not registered in
   {{TRUST-POLICY}} §Subject Authority Extraction Procedures Registry, reject the assertion.
...
- Eliminates dependence on DNS resolution integrity for the
  ...
  {{TRUST-POLICY}} §Subject Authority Determination, but the authorization itself is retrieved
...
within a category, OR-semantics apply ({{TRUST-POLICY}} §Trust Method Categories).
...
Authority Extraction Procedure ({{TRUST-POLICY}} §Subject Authority Extraction Procedures Registry) plus
...
- **Subdomain takeover.** Authority for `email` Subject Identifiers
  ...
  {{TRUST-POLICY}} §Subject Authority Determination.

- **Public-suffix masquerading.** A Subject Identifier whose
  registrable domain is itself a public suffix has no valid
  Subject Authority and is rejected ({{TRUST-POLICY}} §Subject Authority Determination). This
...
- **Profile 3 (signed policy at arbitrary origin)**: the host's
  ...
  Authority's key per {{TRUST-POLICY}} §Signed Policy Metadata; a host
...
or third-party hosting use Publication Profile 3 in
...
part (registrable domain per {{TRUST-POLICY}} §Subject Authority Determination). The local-part of
...
({{TRUST-POLICY}} §Identity Assertion Issuer Trust Methods Registry):
...
Profile Identifier:
: `urn:ietf:params:oauth:authority-delegation-profile:domain-authorized-issuer-discovery`

Authority Holder Form:
: DNS domain (registrable per {{TRUST-POLICY}}
...
Publication Channel:
: DNS TXT at `_oauth-issuer-policy.{authority}` with optional
HTTPS pointer; HTTPS fallback at the authority's well-known URL;
signed-policy publication at arbitrary HTTPS origins via
the signed-policy publication profile ({{publication-profiles}}).
...
`source_selection`
: Subject Authority is extracted from the assertion's Subject
Identifier per {{TRUST-POLICY}} §Subject Authority Determination
(for `email`, the registrable domain). Selection uses one Subject
Identifier per assertion; the function is invariant under other
claims.
...
profile-specified maximum (recommended ≤24h ceiling, {{dii-caching}}).
Negative results follow the consumer's local caching with the
same ceiling. Cumulative Indeterminate retrievals MUST NOT extend
the effective cache lifetime; the maximum exposure window
following a revocation is the sum of positive cache and the
cumulative-indeterminate cap.

`transitivity_limit`
: Depth-1. The `authorized_issuers` list names Assertion Issuers
directly; a Subject Authority cannot delegate authority over its
namespace to other authorities. Federation chains for issuer
AUTHENTICATION remain in scope under the orthogonal
`issuer_authentication` category (defined by {{TRUST-POLICY}}).
