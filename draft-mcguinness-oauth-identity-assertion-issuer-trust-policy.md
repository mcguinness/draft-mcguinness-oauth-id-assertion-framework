...
abbrev: "Identity Assertion Issuer Trust Policy"
docname: draft-mcguinness-oauth-identity-assertion-issuer-trust-policy-latest
category: std
submissiontype: IETF
v: 3
...
This document defines the Identity Assertion Issuer Trust Policy, a
JSON policy document that lets a Resource Authorization Server keep
issuer authentication and subject namespace authorization as distinct,
...
This document profiles the Authority Delegation Framework
({{AUTHORITY-DELEGATION}}) for OAuth identity assertions and is
consumed by {{DAI}}, which defines one Subject-Authority publication
mechanism. The three documents together are described in
{{family}}.

## Trust Evaluation Categories {#two-trust-questions}

This document profiles the two independent trust-evaluation
categories of {{AUTHORITY-DELEGATION}} (§Independent Trust
Evaluation Categories) for OAuth identity assertions:
...
cross-category combination rule ({{AUTHORITY-DELEGATION}}
§Cross-Category Combination Rule) applies as instantiated in
{{rasp}}: evidence from EACH applicable category is required.
...
- **Shared-issuer multi-tenant Identity Providers.** A customer
  ...
  tenant-isolation enforcement implicitly. This policy makes
...
This document targets open-world deployments
({{AUTHORITY-DELEGATION}} §Open-World and Closed-World Delegation):
...
This document profiles {{AUTHORITY-DELEGATION}} for OAuth identity
assertions. The OAuth profile family consists of:

- **{{AUTHORITY-DELEGATION}}**: the abstract Authority Delegation
  pattern (Authority Holder, Delegate, Delegation Artifact,
  Validator; lookup state taxonomy; combination rule; profile
  registry).
- **This document**: the Identity Assertion Issuer Trust Policy
  ({{trust-policy-document}}) published by a Resource Authorization
  Server, the Trust Method machinery, OAuth grant profile bindings,
  and the Subject Authority Determination concept.
- **{{DAI}}**: Domain-Authorized Issuer Discovery, defining the
  Issuer Authorization Policy wire format and two Trust Methods
  (`domain_authorized_issuer`, `https_authorized_issuer`) that
  consume it.
...
The scope of this document is the Trust Policy document, the
Trust Method machinery (categories, combination rule, registry),
the Subject Authority Determination concept and registry, and the
OAuth grant-profile and Actor Profile bindings. Concrete
publication mechanisms for specific Trust Methods
are out of scope and live in dedicated specifications.

The broader Authority Delegation Pattern accommodates attribute
authorities whose authority is not a namespace owner
(government-issued credentials, professional certifications,
employment attestations, decentralized credentials). Implementers
needing attribute attestation outside namespace-bound subject
...
{{extending}} and require no changes to the trust evaluation
model.
...
The `domain_authorized_issuer`, `https_authorized_issuer`, and
`email_verification_dns` Trust Methods are defined in {{DAI}}. They
belong to the `subject_namespace_authorization` category. Profile
authors deploying more than one method in this category SHOULD review
the combination guidance in {{DAI}} to avoid the OR-within-category
downgrade discussed in {{AUTHORITY-DELEGATION}} §Profile Composition
Risks of {{AUTHORITY-DELEGATION}}.
...
The Public Suffix List {{PSL}} is updated continuously. Two
Resource Authorization Servers using PSL snapshots taken at
different times can compute different registrable domains for the
same email address, violating the deterministic source-selection
requirement of {{AUTHORITY-DELEGATION}} §Profile Mapping Matrix
Requirements.

Consumers SHOULD use a PSL snapshot from
`publicsuffix.org` no older than 30 days. Subject Authorities
SHOULD monitor for PSL changes that affect their registrable
domain (typically: introduction of a new public suffix that
shortens or lengthens the computed eTLD+1 for emails in their
namespace). When a PSL change affects a deployed Subject Authority,
operational coordination across cooperating consumers is required
to maintain consistent source selection.

This document does not specify a wire-format mechanism for binding
a Subject Authority to a specific PSL version. Deployments
requiring strict cross-Validator determinism rely on operational
coordination of PSL snapshots.
...
from publishing Subject Authority records that override the
organizational policy at the registrable domain.

This document does not register a subdomain-exact extraction.
Specifications that do so MUST define how the parent
registrable-domain authority delegates to the subdomain authority;
otherwise a subdomain-takeover attacker could publish a competing
policy at the subdomain. A non-normative sketch is provided in
{{extending}}.
...
- The JWT MUST NOT use `alg` value `none`.
- The JWT MUST NOT use a MAC algorithm (HS256, HS384, HS512).
  Trust Policy and Issuer Authorization Policy verification keys
  are necessarily distributed widely; a MAC-based scheme would
  require sharing the signing key with every Validator, defeating
  the signature's authority binding.
- Implementations MUST support ES256 and SHOULD support EdDSA and
  ES384. Implementations MAY support RS256 with key length
  ≥2048 bits; RS256 is permitted for compatibility with existing
  OIDC infrastructure but is less efficient than the elliptic-curve
  alternatives.
- {{RFC8725}} (JSON Web Token Best Current Practices) applies
  unchanged.

The JWT `typ` header MUST identify the signed-policy type to
prevent cross-context confusion attacks per {{RFC8725}} §3.11:

- For Trust Policy documents: `typ: "trust-policy+jwt"`.
- For Issuer Authorization Policy documents:
  `typ: "issuer-authorization-policy+jwt"`.

Consumers MUST reject a `signed_policy` JWT whose `typ` header is
absent or does not match the expected value for the document
being verified.
...
unrecognized members and `crit`. A conflict exists when a member name
appears in both the unsigned outer document and the signed JWT payload
AND the values are not byte-identical after JSON canonicalization.
Consumers MUST reject a policy that contains any such conflict; an
attacker who can modify the outer document but not the signed JWT
otherwise has a lever to inject visible-but-ignored members that may
mislead operators or downstream tooling.
...
This section specifies the normative processing procedure. It
instantiates the abstract verification model of
{{AUTHORITY-DELEGATION}} §Abstract Procedure for OAuth identity
assertions.
...
{{subject-authority-determination}}) applies to direct Subject Identifiers only; it
...
profile decisions within the general taxonomy of
{{AUTHORITY-DELEGATION}} §Bounded vs Chained Transitivity:
...
- The named subject controls the local-part of the email address
  (see {{DAI}} §Email Local-Part Is Not Authenticated).
...
Identifiers ({{subject-authority-determination}}) is a PREREQUISITE for deriving
...
This policy establishes whether an Assertion Issuer is authorized
...
the latency model in {{DAI}} §Operational Considerations) but does not
...
Identifier (see {{subject-authority-determination}} for the determination defined by
...
with available mitigations, is in {{DAI}} §Assertion Issuer Discovery.
...
| `crit` | Critical extension identifiers or policy members | IETF | This document |
...
|-|-|-|-|
| `email` | DNS domain | Direct Subject Identifier only | {{subject-authority-determination}} of this document |

# Extending to New Subject Identifier Formats {#extending}

This document registers one Subject Authority extraction procedure
(`email`, {{subject-authority-determination}}). Additional
`subject_namespace_authorization` Trust Methods, including DNS-based
publication methods such as `domain_authorized_issuer`, are defined by
separate specifications such as {{DAI}}. The policy model is designed
to accommodate additional Subject Identifier formats and publication
mechanisms via future specifications. This section describes the
extension procedure for spec authors.

To extend this policy to a new Subject Identifier format, a future
specification:
...
2. **Selects or registers a Trust Method.** If the computed Subject
   ...
   ({{DAI}} §domain_authorized_issuer) applies without
   ...
   ({{DAI}} §email_verification_dns) is intentionally
...
Issuer Authorization Policy document format ({{DAI}} §Issuer Authorization Policy Document), the
verification flow ({{DAI}} §Verification), the Trust Method category
structure and combination rule ({{trust-method-categories}},
{{rasp}}), and the bounded-transitivity property of namespace
authorization ({{transitive-authz-bounded}}) remain unchanged
across extensions.
...
This policy is complementary to OpenID Federation, not a replacement
...
- This framework's distinct contribution is the
  ...
  Authorization Policy ({{DAI}} §Issuer Authorization Policy Document) that supplies its evidence.
