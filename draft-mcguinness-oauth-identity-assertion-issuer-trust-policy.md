...
Evaluation Categories) for OAuth identity assertions, naming them
`issuer_authentication` (the Authenticity category: is the JWT
`iss` claim a recognized signer?) and `subject_namespace_authorization`
(the Delegation Authority category: has the namespace owner
authorized this issuer?). Each Trust Method belongs to exactly
one of these categories. The cross-category combination rule
({{AUTHORITY-DELEGATION}} §Cross-Category Combination Rule)
applies as instantiated in {{rasp}}: evidence from EACH
applicable category is required.
...
publication mechanisms for specific Trust Methods (such as DAI)
...
The `domain_authorized_issuer` and `https_authorized_issuer` Trust
Methods are defined in {{DAI}}. Both belong to the
`subject_namespace_authorization` category and use the Issuer
Authorization Policy as their Delegation Artifact. They are peer
methods; profile authors deploying both SHOULD review the
combination guidance in {{DAI}} to avoid the OR-within-category
downgrade discussed in {{AUTHORITY-DELEGATION}} §Profile Composition Risks of
{{AUTHORITY-DELEGATION}}.
...
2. Determine the Subject Authority `A` from the `email` Subject
   ...
   {{subject-authority-determination}} (registrable domain after `@`).
...
4. If `negative-authoritative` or `indeterminate`, the Trust Method
   ...
   the failure-handling rules of {{DAI}} §Failure Handling (a verifier MUST
...
6. Compare the JWT `iss` claim to the parsed `iss=` values.
   ...
   ({{DAI}} §domain_authorized_issuer), which expresses
...
Profile Identifier:
: `urn:ietf:params:oauth:authority-delegation-profile:identity-assertion-issuer-trust-policy`

Authority Holder Form:
: Subject Authority. Form is namespace-dependent: for `email`
Subject Identifiers, a DNS domain (registrable per
{{subject-authority-determination}}).

Delegate Form:
: Assertion Issuer (OAuth authorization server identifier; HTTPS URL
compared per {{RFC8414}} §2).

Publication Channel:
: Trust Method-specific. {{DAI}} defines DNS+HTTPS publication for
`domain_authorized_issuer` and `https_authorized_issuer`; other
Trust Methods register their own publication channels.

Subdelegation:
: `subject_namespace_authorization` category is depth-1
(no subdelegation). `issuer_authentication` category permits
chained transitivity (e.g., `openid_federation`).
...
`source_selection`
: The Subject Authority is extracted from the assertion's Subject
Identifier via the procedure registered in
{{iana-authority-registry}}. Source selection uses one Subject
Identifier per assertion; the binding function is invariant under
other claims. Profiles defining additional Trust Methods MAY use
non-namespace source selection but MUST satisfy
{{AUTHORITY-DELEGATION}} §Deterministic Source Selection.

`revocation_lifecycle`
: Per-Trust-Method. {{DAI}} bounds revocation latency by DNS TTL
plus HTTP cache lifetime (recommended ≤24h ceiling).
`openid_federation` follows OpenID Federation revocation rules.
Negative results are not separately cached; absence of a
Delegation Artifact maps to Negative per
{{AUTHORITY-DELEGATION}} §Exception-Handling and Fallback Model.

`transitivity_limit`
: Mixed: depth-1 for `subject_namespace_authorization`, chained
for `issuer_authentication` via `openid_federation`. The
combination rule in {{rasp}} composes both categories AND-wise.

# Extending to New Subject Identifier Formats {#extending}

This document registers one Subject Authority extraction procedure
(`email`, {{subject-authority-determination}}) and one `subject_namespace_authorization`
Trust Method that publishes via DNS (`domain_authorized_issuer`,
{{DAI}} §domain_authorized_issuer). The framework is designed
...
4. The tool provider evaluates the trust policy's
   ...
   a. Extracts the Subject Authority from the assertion's email
      ...
      {{subject-authority-determination}}, the registrable domain of
...
- If the customer's DNS record is missing or unreachable, the
  ...
  {{DAI}} §Failure Handling and the tool provider rejects the assertion.
...
- If `example.com` rotates its authorized agent platform (replaces
  ...
  next cache refresh per {{DAI}} §Caching.
...
({{DAI}} §Single-Issuer Multi-Tenant Identity Providers). This subsection sketches the same
...
cannot express `tenant` ({{DAI}} §Inline-Form Feature Limits):
...
{{DAI}} §Single-Issuer Multi-Tenant Identity Providers; the framework surfaces the customer's choice
